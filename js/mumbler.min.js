...
let mu_msc='<textarea id="mu-say" placeholder=""  contenteditable="true" ></textarea><div id="mu-child"><select id="mu-labels" class="mu-button"><option class="mu-label oneword" value="oneword">💬一言</option><option class="mu-label thinking" value="thinking">💫想法</option><option class="mu-label life" value="life">👻生活</option><option class="mu-label book" value="book">📖书摘</option><option class="mu-label atwork" value="atwork">🍥Atwork</option></select><span id="mu-expression" class="mu-button">表情</span><span id="mu-post" class="mu-button">发布</span></div>'
function Mumbler(e) {
	return new muEvery(e)
}
...
muEvery.prototype.init = function(e) {
	let t = this;
	let i = document.createElement("script");
	// Import file
	return e && t._init(e), t;
}, muEvery.prototype._init = function(e) {
	try {
		AV.init({
			appId: e.appId,
			appKey: e.appKey
		...
		// 获取渲染数据
		muEvery.prototype.muControl();
	} catch (e) {
		console.log(e)
	}
}, muEvery.prototype.muControl = function() {
	let Mu={
		ms:document.getElementById('mu-say-content'),
		mc:document.getElementById('mu-child'),
		rm:document.getElementById('mu-continue'),
		uh:document.getElementById('mu-switch'),
		mu:document.getElementById('mu-user'),
		lo:document.getElementById('mu-login'),
		
	}
	let page = 0;
	muEvery.prototype.getContent(page)
	
	function switchUser(){
		if(AV.User.current()){
			Mu.uh.src='/img/faces.jpeg'
			sw='<span class="mu-input">Hi~'+AV.User.current().attributes.username+'</span>'
			Mu.ms.style.display='flex'
			Mu.ms.innerHTML= mu_msc
			Mu.lo.innerHTML = sw
			return AV.User.current()&&muEvery.prototype.muEdit()
		}else{
			Mu.uh.src='/img/faces.jpeg'
			sw='<input type="text" placeholder="Username" id="mu-username" class="mu-input mu-user" required="required"><input type="password" placeholder="Password" id="mu-pwd" class="mu-input mu-pwd" required="required">'
			Mu.ms.style.display='none'
			Mu.ms.innerHTML= ''
			Mu.lo.innerHTML = sw
			return undefined;
		}
	...
	function changeUser(){
		if(AV.User.current()){
			confirm("是否退出登录?") ? AV.User.logOut()&&switchUser() : '' ;
		}else{
			let user=document.getElementById('mu-username').value.trim(),
				pwd=document.getElementById('mu-pwd').value.trim();
			(!user||!pwd)?'':AV.User.logIn(user, pwd).then((t => { return switchUser() }), (e => { console.log(e) }))
		}
	}
	switchUser()
	Mu.rm.onclick = function() {
		page += 1;
		Mu.rm.innerHTML = ICON.loading;
		return muEvery.prototype.getContent(page)
	},
	//用户登录
	Mu.uh.onclick = function() {
		changeUser()
	}
}, muEvery.prototype.getContent = function(page) {
	let k = new AV.Query('Mumbler'),ps = 3;
	let data = [],arr = [];
	let rm = document.getElementById('mu-continue');
	k.descending("createdAt"), k.limit(ps), k.skip(ps * page)
	k.find().then(function(t) {
		muEvery.prototype.muReady(arr.concat(t))
		if (page > t.length) {
			rm.innerHTML = '到底了~';
			document.getElementById('mu-continue').onclick=null
		} else {
			rm.innerHTML = 'Read More';
		}
	})
}, muEvery.prototype.muReady = function(e) {
	let data = '';
	//处理好获得的数据
	e.forEach(function(t) {
		let Author = 'Lorre',
			id = t.attributes.id,
			date = t.createdAt,
			html = t.attributes.mu_html,
			eq = t.attributes.mu_eq,
			label = t.attributes.mu_label;
		let ml = '',
		o = new Date(date);;
		switch (label) {
			case 'oneword':
		...
		function createdAt() {
			...
			let h = o.getFullYear() + "-" + l(o.getMonth() + 1) + "-" + l(o.getDate()),
				p = l(o.getHours()) + ":" + l(o.getMinutes()) + ":" + l(o.getSeconds()),
				u = o.getHours(),
				g = "";
			return h
		}
		data += '<div class="mu-card"><div class="mu-issue"><span class="mu-author">Lorre<span>' + ICON.badge + '</span></span><span class="mu-label mu-label-' + label + '">' + ml +'</span></div><div class="mu-cont">' + html +'</div><div class="mu-card-body"></div><div class="mu-card-footer"><span class="mu-time">' +createdAt() + '</span><span class="mu-issue-text">·</span><span calss="mu-eq">' + eq +'</span></div></div>'
	})
	return muEvery.prototype.showContent(data)
	// console.log(demo)
}, muEvery.prototype.showContent = function(e) {
	let mcl = document.getElementById('mu-card-list');
	let box = document.createElement("div");
	box.innerHTML = e
	mcl.appendChild(box)
},muEvery.prototype.muEdit=function(){
	let p=document.getElementById('mu-post'),
		s=document.getElementById('mu-expression');
	function mu_getCp(){
		let bs=navigator.appVersion;
		if(bs.indexOf('Window') >=1){
			return '🖥️发布于win11';
		}else if(bs.indexOf('Android') >=1){
			return '📱发布于Android';
		}else{
			return '🍩发布于‍微信公众号'
		}
	...
	p.onclick=function(){
		let	i = document.getElementById("mu-say").value,
			mls= document.getElementById('mu-labels'),
			ml=mls.options[mls.selectedIndex].value;
		if ("" === i.trim()){
			console.log('说说不能为空')
		}else{
			let c = new showdown.Converter;
			c.setOption("strikethrough", 1);
			i = function(e) {
				if (void 0 !== e) {
					for (let t in muBlibili) {
						let i = "[" + t + "]",
							a = "<img class='mu-img mu-bili' src='" + muBlibili[t] + "'/>";
			...
			let o = c.makeHtml(i);
			const MuSay = AV.Object.extend('Mumbler');
			const musay = new MuSay();
			musay.set('mu_eq', mu_getCp());
			musay.set('mu_Md', i);
			musay.set('mu_html', o);
			musay.set('mu_label', ml);
			musay.save().then((promise) => {
				muEvery.prototype.muControl();
			}, (error) => {
			  // 异常处理
			  console.log(error)
